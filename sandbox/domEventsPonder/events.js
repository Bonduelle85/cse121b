...
function renderTasks(tasks) {
  ...
  tasks.forEach( task => 
    todoListElement.innerHTML +=`<li ${task.completed ? 'class="strike"' : ""}>
                                    <p>${task.detail}</p>
                                    <div>
...
function newTask() {
  ...
  const taskDetail = todoElement.value
  let taskObject = {detail: taskDetail,  completed: false};
...
function removeTask(taskElement) {
  ...
  tasks = tasks.filter( task => 
    task.detail != taskElement.childNodes[1].innerText
  );
...
function manageTasks(event) {
  ...
  // We need to get the parent li to work with however. 
  // HINT: Remember element.closest()? Look it up if you don't
  // because we added 'data-function="delete"' to each icon in a task 
  // we can access a dataset property on our target
